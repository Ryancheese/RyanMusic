...
import { QishuiAccount } from './accounts/qishui.ts';
...
import { QishuiService } from './qishui.ts';
  ...
  app.all('/api.php', async (c) => {
    ...
    if (get === 'url') {
      ...
      const resolveCross = async () => {
        const meta = type === 'qishui' ? qishui.meta(id) : undefined;
        const title = String(c.req.query('title') || meta?.title || '').trim();
        const artist = String(c.req.query('artist') || meta?.author || '').trim();
        if (!title) return null;
        const trySource = async (altType: 'netease' | 'qq', query: string, mode: 'strict' | 'titleOnly') => {
        ...
        return (
          await trySource('netease', query, 'strict')
          || await trySource('qq', query, 'strict')
          || (artist ? await trySource('netease', title, 'titleOnly') : null)
          || (artist ? await trySource('qq', title, 'titleOnly') : null)
        );
      };
      ...
      const proxyOpts = {
        ...
        cookie: type === 'netease' ? neteaseCookie : type === 'qq' ? qqCookie : type === 'qishui' ? (qishuiAccount.sessionCookie() || undefined) : undefined,
      };
    ...
    if (get === 'pic') {
      const pic = type === 'qq'
        ? await qq.resolvePicUrl(id)
        : type === 'qishui'
          ? await qishui.publicCover(id)
          : await netease.resolvePicUrl(id);
  ...
  const handleIndex = async (c: any) => {
    ...
    if (c.req.method === 'POST' && xhr) {
      ...
      if (action === 'sign_media') {
        const type = post.type === 'qq' ? 'qq' : post.type === 'qishui' ? 'qishui' : 'netease';
        ...
        const stub: import('./config.ts').Track = {
          type,
          songid: id,
        ...
        const wrapped = type === 'qq' ? qq.wrap(stub) : type === 'qishui' ? qishui.wrap(stub) : netease.wrap(stub);
      ...
      try {
        if (filter === 'name') {
          ...
          const result = type === 'qq'
            ...
            : type === 'qishui'
              ? await qishui.searchByCategory(input, page, activeCategory, qishuiAccount.sessionCookie() || '')
        ...
        if (filter === 'id') {
          const tracks = type === 'qq'
            ? await qq.songsByIds([input])
            : type === 'qishui'
              ? await qishui.songsByIds([input])
              : await netease.songsByIds([input]);

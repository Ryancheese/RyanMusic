...
if (isset($_GET['download']) && isset($_GET['url'])) {
    $download_url = trim($_GET['url']);
    if (!filter_var($download_url, FILTER_VALIDATE_URL)) {
        header('HTTP/1.1 400 Bad Request');
        exit('Invalid url');
    }
    $download_name = isset($_GET['name']) ? $_GET['name'] : 'RyanMusic';
    $download_name = preg_replace('/[\\\\\/:*?"<>|\x00-\x1F]/u', '_', $download_name);
    if (!preg_match('/\.mp3$/i', $download_name)) {
        $download_name .= '.mp3';
    }
    if (!mc_stream_download($download_url, $download_name)) {
